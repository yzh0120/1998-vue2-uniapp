<template>
  <div>
    <uni-forms-item :label="`${text}省`" :name="province" :required="required" v-if="province"
      :rules="[{ required: required, errorMessage: '必填' }]">
      <uni-data-select v-model="provinceValue" :localdata="range1" @change="change1"
        :disabled="disabled"></uni-data-select>
    </uni-forms-item>
    <uni-forms-item :label="`${text}市`" :name="city" :required="required"
      :rules="[{ required: required, errorMessage: '必填' }]" v-if="city">
      <uni-data-select v-model="cityValue" :localdata="range2" @change="change2" :disabled="disabled"></uni-data-select>
    </uni-forms-item>
    <uni-forms-item :label="`${text}区`" :name="areas" :required="areasRequired"
      :rules="[{ required: areasRequired, errorMessage: '必填' }]" v-if="areas && !only2">
      <uni-data-select v-model="areasValue" :localdata="range3" @change="change3"
        :disabled="disabled"></uni-data-select>
    </uni-forms-item>
  </div>
</template>
...
export default {
  ...
  methods: {
    setValue() {
      //   console.log(this.item[this.city],"citycity")
      // console.log(this.item, "items")
      this.provinceValue = this.item[this.province]
      this.cityValue = this.item[this.city]
      this.areasValue = this.item[this.areas]
      if (this.provinceValue) {
        this.setCity(this.provinceValue)
      }
      if (this.cityValue) {
        this.setArea(this.provinceValue, this.cityValue)
      }
    },
    setProvince() {
      this.range1 = []
      for (let key of Object.keys(city)) {
        this.range1.push({
          text: key,
          value: key,
          // children:this.setCity(obj[key])
        })
      }
    },
    setCity(provinceValue) {
      this.range2 = []
      for (let key of Object.keys(city)) {
        if (key == provinceValue) {
          for (let key of Object.keys(city[key])) {
            this.range2.push({
              text: key,
              value: key,
              // children:this.setArea(obj[key])
            })
          }
        }
      }
    ...
    setArea(provinceValue, cityValue) {
      this.range3 = []
      for (let key of Object.keys(city)) {
        if (key == provinceValue) {
          for (let keyCity of Object.keys(city[key])) {
            if (keyCity == cityValue) {
              for (let key of city[key][keyCity]) {
                this.range3.push({
                  text: key,
                  value: key,
                })
