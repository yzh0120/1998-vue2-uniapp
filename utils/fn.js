...
//   let obj = store.state.gm.dataItem[field]
//   if (obj) {
//     return obj.find((e) => {
...
///数据字典转化
export function dataItem_list(field, isNumber = false) {
  let arr = deepClone(store.state.gm.dataItem[field])
  arr.forEach((e) => { 
    e.text= e.label
  })
  if (arr) {
...
export function dataItem_text(field, value, isNumber) {
  ...
  let arr = deepClone(store.state.gm.dataItem[field])//store.state.gm.dataItem[field]
  arr.forEach((e) => { 
    e.text= e.label
  })
  if (arr) {
...
export function getDomInfo (id, that) {
...
/**
 * 判断微信小程序运行设备：电脑（PC）/手机（Mobile）
 * @returns {String} 'pc' | 'mobile' | 'unknown'
 */
export function getWechatDeviceType() {
  // console.log(uni.getEnv().WECHAT_MINIPROGRAM,"uni.getEnv().WECHAT_MINIPROGRAM")
  // console.log(uni.getEnv().WECHAT_MINIPROGRAM,"uni.getEnv().WECHAT_MINIPROGRAM")
  // // 仅在微信小程序端执行，避免跨端报错
  // if (!uni.getEnv().WECHAT_MINIPROGRAM) {
  //   return 'unknown';
  // }
  ...
  try {
    ...
    //   return 'pc'; // 开发环境默认视为PC端
    // }
    // 判断是否为微信PC端（Windows/macOS）
    if (['windows', 'macos',`devtools`].includes(platform)) {
      return 'pc';
    }
    // 判断是否为手机端（iOS/Android）
